% ZI: Great job! Just remember to run the code to make sure it doesn't
% throw any errors, you'll catch a lot of typos that way! Grade: 94
...
A(:, 1) = (-1:-1:-4)';
    ...
    % ZI: perfect! you don't technically need the transpose in the last
    % line, since MATLAB will automatically do that for you if the number
    % of elements match, but it doesn't hurt!
...
C(C < 0) = 0;
    ...
    % ZI: perfect! you can also do sum(C<0,'all'), which might be slightly
    % more efficient than C(:), but it still works!
...
D_2 = sum(C) / size(C, 1);
    ...
    % ZI: perfect! I like to specify the dimension in the mean() and sum()
    % functions even if you're working along the default column dimension,
...
I = reverse_rows(H')';
    ...
    % ZI: perfect! just remember you can't define a function in a script,
    % so next time just create a separate file with the function in it
...
J = [x1; [x2, x3]; x1];
    ...
    % ZI: you've got the right idea, but you need an extra column in the
    % middle block, otherwise it throws an error because x1 has 5 columns
    % and [x2, x3] only has 4. Half points
